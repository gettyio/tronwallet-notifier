...
const valid = {
  issuer: '27UzaKnuqqHmfyzUChETgU1EoBqHy8rRoGk',
...
const invalid = {
  issuer: '27UzaKnuqqHmfyzUChET======WRONG=====gU1EoBqHy8rRoGk',
  from: '27khY3PteHw69bcfx======WRONG=====UVUhFu373UWxJgiycV',
  token: 'T======WRONG=====RX',
  amount: '12======WRONG=====34'
};

describe('API Wallet Participate Endpoints', () => {
  it('should return error message when send an invalid "from" field', (done) => {
    request
      .post('/v1/wallet/participate')
  ...
  it('should return error message when send an invalid "issuer" field', (done) => {
    request
      .post('/v1/wallet/participate')
      .set('Accept', 'application/json')
      .expect('Content-Type', /json/)
      .send({ ...valid, issuer: invalid.issuer })
      .end((err, res) => {
        const errorStack = JSON.parse(res.error.text);
        ...
        expect(errorStack.message).to.equal('child "issuer" fails because ["issuer" Issuer address is invalid]');
